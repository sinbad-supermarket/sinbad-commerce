create or replace function public.can_view_vendor_submission_image_object(p_path text)
returns boolean
language plpgsql
stable
security definer
...
begin
  ...
  return public.is_admin()
    or (
      public.is_active_vendor_member(parsed_vendor_id)
      and exists (
        select 1
        from public.product_review_submissions prs
        where prs.id = parsed_submission_id
          and prs.vendor_id = parsed_vendor_id
      )
    );
end;
$$;
...
create policy "Vendor members can read staged submission image objects"
on storage.objects
for select
to authenticated
using (
  bucket_id = 'product-images'
  and public.can_view_vendor_submission_image_object(name)
);

create policy "Vendor members can upload staged submission image objects"
on storage.objects
for insert
to authenticated
with check (
  bucket_id = 'product-images'
  and public.can_manage_vendor_submission_image_object(name)
);

create policy "Vendor members can update staged submission image objects"
on storage.objects
for update
to authenticated
using (
...
create policy "Vendor members can delete staged submission image objects"
on storage.objects
for delete
to authenticated
using (
...
begin
  ...
  if primary_image_count > 1 then
    raise exception 'Only one primary image is allowed.';
  end if;
  ...
  if exists (
    ...
    where nullif(image_item->>'id', '') is null
      ...
      or (image_item->>'width' is not null and (image_item->>'width')::integer <= 0)
      or (image_item->>'height' is not null and (image_item->>'height')::integer <= 0)
  ...
  select
    ...
    case
      when image_item->>'width' is null then null
      else (image_item->>'width')::integer
    end,
    case
      when image_item->>'height' is null then null
      else (image_item->>'height')::integer
    end,
...
revoke all on function public.can_view_vendor_submission_image_object(text) from public;
revoke all on function public.can_manage_vendor_submission_image_object(text) from public;
revoke all on function public.approve_product_review_submission(uuid) from public;

grant execute on function public.can_view_vendor_submission_image_object(text) to authenticated;
grant execute on function public.can_manage_vendor_submission_image_object(text) to authenticated;
