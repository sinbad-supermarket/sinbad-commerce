...
drop function if exists public.is_canonical_product_field_available(text, text, uuid);
...
begin
  ...
  if normalized_field = 'barcode' then
    return true;
  end if;

  if normalized_field = 'sku' and p_vendor_id is null then
    raise exception 'Vendor is required when checking SKU availability.';
  end if;
  ...
  select exists (
    ...
    where (
      ...
      or (
        normalized_field = 'sku'
        and p.vendor_id = p_vendor_id
        and p.sku = normalized_value
      )
    )
...
revoke all on function public.is_canonical_product_field_available(text, text, uuid, uuid)
  from public;
grant execute on function public.is_canonical_product_field_available(text, text, uuid, uuid)
  to authenticated;
...
begin
  ...
  if normalized_sku is not null and exists (
    select 1
    from public.products p
    where p.vendor_id = submission_record.vendor_id
      and p.sku = normalized_sku
      and (submission_record.product_id is null or p.id <> submission_record.product_id)
  ) then
    raise exception 'This SKU already exists for one of your products.';
  end if;
